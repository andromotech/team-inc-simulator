...
d3.json("./incidents/general_incidents.json", function (error, data) {
    ...
    container.on("click", spin1);

    function spin1(d) {
        container.on("click", null);
        //all slices have been seen, all done
        if (oldpick1.length == data.length) {
            container.on("click", null);
            return;
        }
        ...
        picked1 = Math.round(data.length - (rotation % 360) / ps);
        picked1 = picked1 >= data.length ? (picked1 % data.length) : picked1;
        ...
        vis1.transition()
            ...
            .each("end", function () {
                //mark incident as seen
                 d3.select("#wheel .slice:nth-child(" + (picked1 + 1) + ") path")
                    .attr("fill", "#111");
                // if Ink story is provided, override the actual scenario.
                if (data[picked1].inkstory != undefined) {
                    //populate incident
                    d3.select("#incident p")
                        .html("<h4 class=\"f4 center mw6\">" + data[picked1].title + "</h4>" + "<div id=\"play-area\"> <ol id =\"choices\"><ol></div>");
                    container.on("click", spin1, stopwatch.reset(), stopwatch.start(), changeControls(), loadStory(data[picked1].inkstory));
                   } else {
                    //populate incident
                    d3.select("#incident p")
                        .html("<h4 class=\"f4 center mw6\">" + data[picked1].title + "</h4>" + data[picked1].scenario);
                    container.on("click", spin1, stopwatch.reset(), stopwatch.start(), changeControls());
                }
