...
d3.json("./incidents/oncalls.json", function (error, data) {
    if (error) throw error;
    var svg = d3.select("#wheel1")
        .append("svg")
        .data([data])
        ...
        vis.transition()
            ...
            .each("end", function () {
                ...
                if (data[picked].inkstory != undefined) {
                    //populate incident
                    d3.select("#whosoncall p")
                        .html("<h4 class=\"f4 center mw6\">" + data[picked].title + "</h4>" + "<div id=\"play-area\"> <ol id =\"choices\"><ol></div>");
                    container.on("click", spin, stopwatch.reset(), stopwatch.start(), changeControls(), loadStory(data[picked].title));
                   } else {
                    //populate incident
                    d3.select("#whosoncall p")
